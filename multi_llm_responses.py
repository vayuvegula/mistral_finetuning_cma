...
def generate_questions(text: str, provider: str, model: str) -> tuple:
    try:
        ...
        if provider == "anthropic":
            ...
            # No known method for token usage in Mistral, so we'll set it to 0
            tokens_used = 0
            return response.choices[0].message.content, tokens_used
        else:
...
if __name__ == "__main__":
    main(
        ...
        model="gpt-4o-mini"
        # or "gpt-3.5-turbo" for OpenAI, "mistral-large-latest" for Mistral, or "claude-3-sonnet-20240229"
    )
